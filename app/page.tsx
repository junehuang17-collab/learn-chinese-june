    ...
    <main className="home-redesign">
      ...
      <section className="home-intro">
        <div className="home-intro-inner">
          <p className="section-kicker">WHY PRACTICAL MANDARIN?</p>
          <h2>Not Chinese for the classroom.<br /><em>Chinese for real life.</em></h2>
          <p>When you arrive in China, you don't need more textbook dialogues. You need to know what to say when the taxi driver calls, the waiter asks a question, or someone starts a conversation.</p>
        </div>
      </section>
      ...
      <section className="section about-section">
        <div className="section-inner about-grid">
          <div className="about-image-stack"><img className="about-photo" src="/june.jpg" alt="June, Mandarin teacher" /><span>JUNE · CTCSOL CERTIFIED</span></div>
      ...
      <section className="section why-section" id="why-june">
        <div className="section-inner">
          <p className="section-kicker">THE JUNE METHOD</p>
        ...
        <div className="section-inner">
          ...
          <div className="path-grid">{siteConfig.lessons.map((lesson, index) => (
            <article className={`path-card ${index === 0 ? "featured" : ""}`} key={lesson.title}>
              <p className="path-eyebrow">{lesson.eyebrow}</p><h3>{lesson.title}</h3><p>{lesson.description}</p>
      ...
      <section className="section resources-section" id="resources">
        <div className="section-inner">
          ...
          <div className="resource-shell">
            <article className="resource-panel"><span className="resource-badge">FREE RESOURCES</span><h3>Learn something useful today.</h3><p>Short, practical mini-lessons with Chinese, pinyin, English and real-life examples.</p><div className="resource-list">{freeResources.map((item) => <Link href="/materials" key={item}>{item}</Link>)}</div><Link className="button button-dark" href="/materials">Explore free resources →</Link></article>
            <article className="resource-panel paid"><span className="resource-badge">PAID MATERIALS</span><h3>Build your own Chinese toolkit.</h3><p>Visual, scenario-based materials you can review, reuse and take with you before your next trip or conversation.</p><div className="resource-list">{siteConfig.materials.slice(0,4).map((item) => <a href={siteConfig.payhip} target="_blank" rel="noreferrer" key={item.title}>{item.icon} {item.title} ↗</a>)}</div><a className="button button-light" href={siteConfig.payhip} target="_blank" rel="noreferrer">Browse paid materials →</a></article>
          </div>
        </div>
      </section>

      <section className="section section-soft review-home">
        <div className="section-inner">
          <div className="review-intro"><div><p className="section-kicker">STUDENT VOICES</p><h2 className="section-heading">Real learners. <em>Real progress.</em></h2></div><div className="review-score-box"><strong>{siteConfig.reviewStats.rating}</strong><span>★★★★★ · {siteConfig.reviewStats.reviews} reviews</span></div></div>
          <div className="review-cards">{siteConfig.reviews.slice(0,3).map((review) => <article className="review-card-home" key={review.label}><blockquote>“{review.quote}”</blockquote><footer>{review.label}</footer></article>)}</div>
          <div className="review-link"><Link className="text-link" href="/reviews">Read more student reviews →</Link></div>
        </div>
      </section>

      <section className="final">
        <p className="section-kicker">READY TO USE CHINESE?</p><h2>Your next conversation starts here.</h2><p>Tell me what you want to be able to do in Chinese. We'll build the lesson around your real life.</p>
